...
def RF_test(clf, Xtest,ytest):
    ba = clf.predict(Xtest);ba = resort(ba)
    acc = np.abs(ba - ytest)
    acd = 1-(np.count_nonzero(acc)/len(ytest))
    return acd,ba
    #return ba
...
def DNN_test(clf, Xtest,ytest):
    ba = clf.predict(Xtest)
    print(ba.shape)
    ba = np.argmax(ba, axis=1);ba = resort(ba)
    #print(ba)
    #print(ytest)
    acc = (ba-ytest);
    acd = 1- (np.count_nonzero(acc)/ len(ytest) )
...
print('time in advance (quadruple,second instance)', no)
...
acc,acc_ =  RF_test(clf, Xtest, rosalie_label_gd[le:len(rosalie_label_gd):]  )
print('random forest',acc)
precision, recall, fscore, support = score(rosalie_label_gd[le:len(rosalie_label_gd):] , acc_)
print('fscore: {}'.format(fscore))

clf = SV_train(Xtrain,ytrain)
acc,acc_ = SV_test(clf, Xtest, rosalie_label_gd[le:len(rosalie_label_gd):] )
...
acc,acc_ = LR_test(clf, Xtest, rosalie_label_gd[le:len(rosalie_label_gd):] )
...
acc,acc_ = DNN_test(clf, Xtest, rosalie_label_gd[le:len(rosalie_label_gd):] )
print('DNN',acc)
precision, recall, fscore, support = score(rosalie_label_gd[le:len(rosalie_label_gd):] , acc_)
print('fscore: {}'.format(fscore))
